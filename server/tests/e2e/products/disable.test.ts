...
describe("Product (E2E) - PATCH /products/disable/:id", () => {
  ...
  it("should be able to disable a product", async () => {
    ...
    const response = await request(app).patch(`/products/disable/${product.id}`)
  ...
  it("should not be able to disable a product that does not exist", async () => {
    ...
    const response = await request(app)
      .patch(`/products/disable/${nonExistingProductId}`)
      .set("Authorization", `Bearer ${token}`);

    expect(response.status).toBe(StatusCodes.NOT_FOUND);
  });

  it("should not be able to disable a product if the token is invalid", async () => {
    const product = await productsRepository.create("Product 1", null, 10, null);
    const token = "invalid-token";

    const response = await request(app)
      .patch(`/products/disable/${product.id}`)
      .set("Authorization", `Bearer ${token}`);

    expect(response.status).toBe(StatusCodes.UNAUTHORIZED);
  });

  it("should not be able to disable a product if the token is missing", async () => {
    const product = await productsRepository.create("Product 1", null, 10, null);
  ...
  it("should not be able to disable a product if the user is not admin", async () => {
