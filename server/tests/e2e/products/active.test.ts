...
describe("Product (E2E) - PATCH /products/active/:id", () => {
  ...
  it("should be able to active a product", async () => {
    ...
    const response = await request(app).patch(`/products/active/${product.id}`)
  ...
  it("should not be able to active a product that does not exist", async () => {
    ...
    const response = await request(app)
      .patch(`/products/active/${nonExistingProductId}`)
      .set("Authorization", `Bearer ${token}`);

    expect(response.status).toBe(StatusCodes.NOT_FOUND);
  });

  it("should not be able to active a product if the token is invalid", async () => {
    const product = await productsRepository.create("Product 1", null, 10, null);
    const token = "invalid-token";

    const response = await request(app)
      .patch(`/products/active/${product.id}`)
      .set("Authorization", `Bearer ${token}`);

    expect(response.status).toBe(StatusCodes.UNAUTHORIZED);
  });

  it("should not be able to active a product if the token is missing", async () => {
    const product = await productsRepository.create("Product 1", null, 10, null);
  ...
  it("should not be able to active a product if the user is not admin", async () => {
